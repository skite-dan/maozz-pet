...
router.get('/sitemap.xml', async (req, res, next) => {
  try {
    const posts = await query(`SELECT id, slug, title, updated_at FROM posts WHERE status = 'published' ORDER BY updated_at DESC LIMIT 1000`);
    const staticPages = [
...
router.get('/', async (req, res, next) => {
  try {
    ...
    res.render('index', { title: '毛茸茸星球-专业铲屎官宠物交流论坛 | 养猫养狗饲养问诊同城领养', metaDesc: '毛茸茸星球是铲屎官专属宠物社区，分享猫狗饲养干货、宠物疾病问诊、同城遛宠、无偿领养、宠物用品闲置交流，百万养宠经验免费查阅，一站式解决养宠难题。', metaKeywords: '宠物论坛,养猫攻略,养狗教程,猫咪疾病,同城宠物领养,宠物交流社区', hotPosts, latestPosts, forumPosts, banners, stats });
  } catch (err) { next(err); }
});
